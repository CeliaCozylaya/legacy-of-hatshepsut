...
export class GlyphMap0 {
    ...
    static inf(glyph: Glyph): GlyphInf0 {
        return (glyph in GlyphMap0.glyphs) ? GlyphMap0.glyphs[glyph] : GlyphMap0.bad;
    }
    ...
    static add(c: string, g: Glyph): void {
        let inf: GlyphInf0 = new GlyphInf0(g, c);
        GlyphMap0.warn(g);
        GlyphMap0.glyphs[g] = inf;
    }
    static warn(g: Glyph): void {
        if (GlyphMap0.glyphs.length == g) {
            return;
        }
        console.log(g, 'differs from', GlyphMap0.glyphs.length);
    }
}